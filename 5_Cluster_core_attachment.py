...
if __name__ == "__main__":
    ...
    f = open("dataset/gavin_attr_sim224.txt", "r") #格式：node1 node2 float#"dataset/biogrid_attr_sim96.txt", "r"
    f_protein_out = open(protein_out_file, "w") #输出文件的名字
    Dic_map = {}
    index = 0
    Node1 = []
    Node2 = []
...
#下面这里是调用两个exe执行文件，来完成相应功能。把（）里面的东西当做命令行，输入给windows系统即可。#需要在老电脑运行
    os.system(
        "ConvertPPI.exe " + "dataset/gavin.txt" + " " + protein_out_file + " " + ppi_pair_file + " " + ppi_matrix_file)#dataset/biogrid.txt
    os.system(
